...
|---|---|
| [`offline/`](offline/) | **Offline Implementation**: Operates on a presynaptic voltage recording (CSV) in a **unidirectional** setup with a **Hindmarsh-Rose** neuronal model as the postsynaptic neuron. Because it does not wait for real time, this implementation is extremely fast, completing 400 evaluations in around **30 seconds**. |
...
Third-party dependencies vary by version. The main ones are listed below (refer to each subdirectory's README for details):
...
[1] J. Golowasch, M. Casey, L. F. Abbott, and E. Marder, "Network stability from activity-dependent regulation of neuronal conductances," *Neural Computation*, vol. 11, pp. 1079–1096, 1999.
